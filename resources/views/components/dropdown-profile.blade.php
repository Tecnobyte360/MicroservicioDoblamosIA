...
<div class="relative inline-flex" x-data="{ open: false }">
    ...
    >
        ...
        <ul>
            <li>
                <a class="font-medium text-sm text-gray-500 hover:text-red-600 dark:hover:text-violet-400 flex items-center py-1 px-3" href="{{ route('profile.show') }}" @click="open = false" @focus="open = true" @focusout="open = false">Autogestión</a>
            </li>
            <li>
                <form method="POST" action="{{ route('logout') }}" x-data>
                    @csrf

                    <a class="font-medium text-sm text-gray-500 hover:text-red-600 dark:hover:text-violet-400 flex items-center py-1 px-3"
                        href="{{ route('logout') }}"
