...
class InventarioDisponibleController extends Controller
{
     ...
     *
     * ✅ 1) Búsqueda por palabras (sin armar $filter):
     *    /api/sap/inventario-disponible/query?q=laminas hr 12mm
     *
     * ✅ 2) OData manual:
     *    /api/sap/inventario-disponible/query?$filter=contains(DescripcionArticulo,'LAMINA')&$top=100
     */
    public function query(Request $request): JsonResponse
    {
        try {
            $inventario = $this->consultarInventarioSAPConQuery($request);
            $sentQuery  = $this->buildODataQueryParams($request); // lo que se envió a SAP
    ...
    /**
     * Consulta INVENTARIO_DISPONIBLE_IA desde SAP Service Layer (con filtros OData o q=palabras)
     */
    ...
    /**
     * Permite solo parámetros OData típicos y además:
     * ✅ Si viene ?q=..., arma un $filter por palabras (si NO vino $filter manual).
     */
    private function buildODataQueryParams(Request $request): array
    {
        ...
        foreach ($allowed as $key) {
            ...
            // Soporte por si el cliente manda %24filter/%24select (a veces pasa)
            if ($val === null) {
                $val = $request->query(str_replace('$', '%24', $key));
            }
        ...
        // ✅ Si no viene $filter, pero sí viene q=..., armamos filtro por palabras
        ...
        if ($q !== '' && empty($params['$filter'])) {
            ...
            foreach ($palabras as $p) {
                ...
                // 🔥 Busca por descripción y también por código
                $parts[] = "(contains(DescripcionArticulo,'{$pSafe}') or contains(ItemCode,'{$pSafe}'))";
            }
